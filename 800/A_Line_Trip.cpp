...
public:
    void solve() {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (!i) ans = max(ans, vec[i]);
        else ans = max(ans, vec[i] - vec[i - 1]);

      }

      // 2 * vec[n - 1] is the dist of last station to x and from x to last station. Since there will be no gas station at x.
